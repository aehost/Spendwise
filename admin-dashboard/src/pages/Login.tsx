...
export default function Login() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const data = await post<{ accessToken: string; refreshToken: string; user: { id: string; email: string; role: string } }>('/auth/login', { email, password })
      if (!['admin', 'support'].includes(data.user.role)) {
        setError('Access denied. Admin or support role required.')
        setLoading(false); return
      }
      localStorage.setItem('sw_admin_token', data.accessToken)
      localStorage.setItem('sw_admin_user', JSON.stringify(data.user))
      nav('/')
    } catch (e: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <div className="text-2xl font-black text-white">SpendWise Admin</div>
          <div className="text-sm text-gray-500 mt-1">Management Dashboard</div>
        </div>
        <form onSubmit={handleSubmit} className="bg-card rounded-2xl p-6 border border-border">
          ...
          <p className="text-xs text-gray-600 text-center mt-4">Default: admin@spendwise.app / Admin@SpendWise2025</p>
        </form>
      </div>
    </div>
  )
